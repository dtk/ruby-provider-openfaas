...
class Handler
    def run(req)
        response = ArbiterGRPCServer.new.process(req, nil)
        return response
    end
end
...
class ArbiterGRPCServer #< Dtkarbiterservice::ArbiterProvider::Service
  ...
  begin
    response = {}
    if provider_message.class == Hash
      provider_message_hash = provider_message
    else
      provider_message_hash = JSON.parse(provider_message)
    end
    request_type = provider_message_hash['request_type'] || 'default'
...
### DTK debug message: In order to debug your ruby provider module, please step into next line ###
  ...
  def error_response(exception)
    error = ::DTKModule::DTK::Error.new({:error_message=>exception.message, :backtrace=>exception.backtrace})
    error.hash_form
  end

  def start_debugger(port=8989, wait_connection = true, bind_ip = '0.0.0.0')
    require 'byebug'
    require 'byebug/core'
    Byebug.wait_connection = wait_connection
    Byebug.start_server(bind_ip, port)
    debugger
  end
